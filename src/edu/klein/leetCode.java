...
public class leetCode {
     ...
     * <p>
     * 只有在爱丽丝在游戏中取得胜利时才返回 true 。假设两个玩家都以最佳状态参与游戏。 输入：n = 3 输出：false 解释：爱丽丝选择 1，鲍勃也选择 1，然后爱丽丝无法进行操作。
     *
     * @param n
     * @return
     */
     ...
     * <p>
     * 输入：rows = 2, cols = 3, rCenter = 1, cCenter = 2 输出：[[1,2],[0,2],[1,1],[0,1],[1,0],[0,0]] 解释：从
     * (r0, c0) 到其他单元格的距离为：[0,1,1,2,2,3] 其他满足题目要求的答案也会被视为正确，例如 [[1,2],[1,1],[0,2],[1,0],[0,1],[0,0]]。
     *
     * @param rows
     * @param cols
     * @param rCenter
...
//        参考了答案
        ...
        Arrays.sort(res, new Comparator<int[]>() {
            ...
            public int compare(int[] o1, int[] o2) {
                return (Math.abs(o1[0] - rCenter) + Math.abs(o1[1] - cCenter)) - (
                    Math.abs(o2[0] - rCenter) + Math.abs(o2[1] - cCenter));
            }
        });
        return res;
    }
     ...
     * <p>
     * 输入：words = ["hello","world","leetcode"], chars = "welldonehoneyr" 输出：10 解释： 可以形成字符串 "hello" 和
     * "world"，所以答案是 5 + 5 = 10。
     *
     * @param words
     * @param chars
     ...
     * <p>
     * 参考了答案: 思路: 判断三点是否同直线，直接思路就是判断斜率。 假设三点分别为a(x1, y1), b(x2, y2), c(x3,y3), a、b两点的斜率为 k1 = (y2 -
     * y1) / (x2 - x1) a、c两点的斜率为 k2 = (y3 - y1) / (x3 - x1) 如果在同一直线，则k1 = k2，考虑到分母为0
     * 的情况，可以直接交叉相乘，省去判断0的情况，直接判断 (y2 - y1) * (x3 - x1) 与 (y3 - y1) * (x2 - x1) 不相等即为不在同一直线上
     *
     * @param points
     * @return
     */
    ...
    public boolean isValid(String s) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            if (!stack.isEmpty() && (stack.peek() + 1 == bytes[i]
                || stack.peek() + 2 == bytes[i])) {
                stack.pop();
            } else {
                stack.push(bytes[i]);
            }
...
//        return left;
    ...
    /**
     * 实现strStr()函数。 给你两个字符串haystack 和 needle ，请你在 haystack 字符串中找出 needle 字符串出现的第一个位置（下标从 0
     * 开始）。如果不存在，则返回 -1 。 说明： 当needle是空字符串时，我们应当返回什么值呢？这是一个在面试中很好的问题。 对于本题而言，当needle是空字符串时我们应当返回 0
     ...
     * <p>
     * 子数组 是数组中的一个连续部分。 输入：nums = [-2,1,-3,4,-1,2,1,-5,4] 输出：6 解释：连续子数组 [4,-1,2,1] 的和最大，为 6 。
     *
     * @param nums
     * @return
     */
...
//        return res;
     ...
     * <p>
     * 提交次数:5 解决方式:参考答案后自己解决 未来是否需要更优化的解题方法:是
     *
     * @param str1
     * @param str2
     * @return
...
//        return res;
     ...
     * 输出：["girl","student"]
     * <p>
     * 提交次数:1 解决方式:自己一次解决 未来是否需要更优化的解题方法:否
     *
     * @param text
     * @param first
     * @param second
    ...
    public String[] findOcurrences(String text, String first, String second) {
        ...
        for (int i = 0; i < s.length - 1; i++) {
            if (s[i].equals(first) && s[i + 1].equals(second) && (i + 2) < s.length) {
                res.add(s[i + 2]);
            }
        }
     ...
     * <p>
     * 提交次数:3 解决方式:自己一次解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     *
     * @param heights
     * @return
     */
...
//        return res;
     ...
     * <p>
     * 提交次数:1 解决方式:自己一次解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param s
     */
     ...
     * <p>
     * 提交次数:2 解决方式:自己一次解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     * @return
     */
     ...
     * <p>
     * 请注意 ，必须在不复制数组的情况下原地对数组进行操作。 输入: nums = [0,1,0,3,12] 输出: [1,3,12,0,0]
     * <p>
     * 提交次数:2 解决方式:自己解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     */
     ...
     * <p>
     * 提交次数:1 解决方式:自己解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     *
     * @param nums
     * @return
     */
    ...
    /**
     * 给定一个非空整数数组，除了某个元素只出现一次以外，其余每个元素均出现两次。找出那个只出现了一次的元素。 输入: [4,1,2,1,2] 输出: 4
     * <p>
     * 提交次数:1 解决方式:自己解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     * <p>
     * 官网思路: 任何数和 00 做异或运算，结果仍然是原来的数，即 a⊕0=a。 任何数和其自身做异或运算，结果是 00，即 a⊕a=0。 异或运算满足交换律和结合律，即
...
//        return 0;
     ...
     * 输入：n = 3 输出：["((()))","(()())","(())()","()(())","()()()"]
     * <p>
     * 难度:中 提交次数:1 解决方式:参考答案 未来是否需要更优化的解题方法:是 优化方向:回溯 未来是否需要复盘:是
     *
     * @param n
     * @return
     */
     ...
     * <p>
     * 提交次数:1 解决方式:自己解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param digits
     * @return
     */
     ...
     * 「快乐数」 定义为：
     * <p>
     * 对于一个正整数，每一次将该数替换为它每个位置上的数字的平方和。 然后重复这个过程直到这个数变为 1，也可能是 无限循环 但始终变不到 1。 如果这个过程 结果为
     * 1，那么这个数就是快乐数。 如果 n 是 快乐数 就返回 true ；不是，则返回 false
     * <p>
     * 输入：n = 19 输出：true 解释： 1^2 + 9^2 = 82 8^2 + 2^2 = 68 6^2 + 8^2 = 100 1^2 + 0^2 + 0^2 = 1
     * <p>
     * 提交次数:3 解决方式:自己解决 未来是否需要更优化的解题方法:是 未来是否需要复盘:是 优化思路:1.哈希集合检测循环,2.快慢指针法
     *
     * @param n
     * @return
     */
    ...
    /**
     * 给你一个整数数组 nums 。如果任一值在数组中出现 至少两次 ，返回 true ；如果数组中每个元素互不相同，返回 false 。 输入：nums =
     * [1,1,1,3,3,4,3,2,4,2] 输出：true
     * <p>
     * 提交次数:2 解决方式:参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     * @return
     */
     ...
     * <p>
     * 提交次数:1 解决方式:自我完成 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     * @return
     */
     ...
     * <p>
     * 提交次数:3 解决方式:自我完成 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param s
     * @return
     */
     ...
     * <p>
     * answer[i] == "FizzBuzz" 如果 i 同时是 3 和 5 的倍数。 answer[i] == "Fizz" 如果 i 是 3 的倍数。 answer[i] ==
     * "Buzz" 如果 i 是 5 的倍数。 answer[i] == i （以字符串形式）如果上述条件全不满足。
     * <p>
     * <p>
     ...
     * <p>
     * 提交次数:2 解决方式:自我完成 未来是否需要更优化的解题方法:是 未来是否需要复盘:否
     *
     * @param s
     * @return
     */
     ...
     * ，应忽略。nums2 的长度为 n
     * <p>
     * 输入：nums1 = [1,2,3,0,0,0], m = 3, nums2 = [2,5,6], n = 3 输出：[1,2,2,3,5,6] 解释：需要合并 [1,2,3] 和
     * [2,5,6] 。 合并结果是 [1,2,2,3,5,6] ，其中斜体加粗标注的为 nums1 中的元素。
     * <p>
     * 提交次数:2 解决方式:自我完成 未来是否需要更优化的解题方法:是 未来是否需要复盘:否
     *
     * @param nums1
     ...
     * <p>
     * 输入：[2, 2, 3, 1] 输出：1 解释：注意，要求返回第三大的数，是指在所有不同数字中排第三大的数。 此例中存在两个值为 2 的数，它们都排第二。在所有不同数字中排第三大的数为
     * 1 。
     * <p>
     * 提交次数:2 解决方式:自我完成 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     * @return
     */
     ...
     * <p>
     * 提交次数:5 解决方式:自我完成 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param s
     * @return
     */
     ...
     * <p>
     * 提交次数:7 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     *
     * @param s
     * @return
     */
...
//    }
    ...
    /**
     * 给你一个字符串 sequence ，如果字符串 word 连续重复 k 次形成的字符串是 sequence 的一个子字符串，那么单词 word 的 重复值为 k 。单词 word 的
     * 最大重复值 是单词 word 在 sequence 中最大的重复值。如果 word 不是 sequence 的子串，那么重复值 k 为 0 。
     * <p>
...
//        return sequence.contains(word)?(sequence.length()-sequence.replace(word, "").length())/word.length():0;
     ...
     * <p>
     * 提交次数:3 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     * @param k
     * @return
     ...
     * <p>
     * 提交次数:3 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param pattern
     * @param s
     * @return
     ...
     * <p>
     * 提交次数:5 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param arr
     * @return
     */
     ...
     * <p>
     * 提交次数:9 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param s
     * @param t
     * @return
     ...
     * <p>
     * <p>
     * 提交次数:3 解决方式: 自己解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param n
     * @return
     */
     ...
     * <p>
     * <p>
     * 提交次数:8 解决方式: 自己解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param s
     * @return
     */
     ...
     * <p>
     * 输入：s = "ubv?w" 输出："ubvaw" 解释：该示例共有 24 种解决方案，只有替换成 "v" 和 "w" 不符合题目要求。因为 "ubvvw" 和 "ubvww"
     * 都包含连续重复的字符。
     * <p>
     * <p>
     * 提交次数:2 解决方式: 自己解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
            ...
            if (s.charAt(i) == '?') {
                ...
                if (i == 0) {
                    if (s.charAt(i + 1) == '?') {
                        ...
                        replacement =
                            (char) (s.charAt(i + 1) < 122 ? s.charAt(i + 1) + 1 : 97) + "";
                    }
     ...
     * <p>
     * 输入：words = ["a", "banana", "app", "appl", "ap", "apply", "apple"] 输出："apple" 解释："apply" 和
     * "apple" 都能由词典中的单词组成。但是 "apple" 的字典序小于 "apply"
     * <p>
     * 提交次数:1 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     *
     * @param words
     ...
     * <p>
     * <p>
     * 提交次数:3 解决方式: 自己有解决方案 答案更高效 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param s
     * @return
     */
    ...
    private void strAppend(String s, StringBuffer res, Stack<Character> charList,
        Stack<Character> numList) {
        boolean flag = true;
        for (int i = 0; i < s.length(); i++) {
            if (flag) {
                if (!charList.isEmpty()) {
...
//        return res;
        ...
        int len = nums.length;
        max = Math
            .max(nums[len - 1] * nums[len - 2] * nums[len - 3], nums[0] * nums[1] * nums[len - 3]);
        return max;
    }
     ...
     * <p>
     * 输入：nums = [1,3,5,4,7] 输出：3 解释：最长连续递增序列是 [1,3,5], 长度为3。 尽管 [1,3,5,7] 也是升序的子序列, 但它不是连续的，因为 5 和
     * 7 在原数组里被 4 隔开。
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     * @return
     */
     ...
     * <p>
     * 输入：s = "011101" 输出：5 解释： 将字符串 s 划分为两个非空子字符串的可行方案有： 左子字符串 = "0" 且 右子字符串 = "11101"，得分 = 1 + 4 =
     * 5 左子字符串 = "01" 且 右子字符串 = "1101"，得分 = 1 + 3 = 4 左子字符串 = "011" 且 右子字符串 = "101"，得分 = 1 + 2 = 3
     * 左子字符串 = "0111" 且 右子字符串 = "01"，得分 = 1 + 1 = 2 左子字符串 = "01110" 且 右子字符串 = "1"，得分 = 2 + 1 = 3
     * <p>
     * 提交次数:3 解决方式: 自我解决 未来是否需要更优化的解题方法:是 未来是否需要复盘:否
     *
     * @param s
     * @return
     */
     ...
     * <p>
     * 提交次数:2 解决方式: 自我解决 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param s
     * @return
     */
     ...
     * <p>
     * 输入：nums = [-4,-2,1,4,8] 输出：1 解释： -4 到 0 的距离为 |-4| = 4 。 -2 到 0 的距离为 |-2| = 2 。 1 到 0 的距离为 |1|
     * = 1 。 4 到 0 的距离为 |4| = 4 。 8 到 0 的距离为 |8| = 8 。 所以，数组中距离 0 最近的数字为 1 。
     * <p>
     * 提交次数:3 解决方式: 自我解决 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param nums
     * @return
     */
     ...
     * <p>
     * arr.length >= 3 在 0 < i < arr.length - 1 条件下，存在 i 使得： arr[0] < arr[1] < ... arr[i-1] < arr[i]
     ...
     * <p>
     * 提交次数:4 解决方式: 自我解决+看答案优化 未来是否需要更优化的解题方法:是 未来是否需要复盘:否
     *
     * @param arr
     * @return
     */
     ...
     * <p>
     * <p>
     * 提交次数:5 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     * @param k
     * @return
     ...
     * isBadVersion(4) -> true 所以，4 是第一个错误的版本。
     * <p>
     * 提交次数:5 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     *
     * @param n
     * @return
     */
     ...
     * <p>
     * 提交次数:4 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     *
     * @param n
     * @return
     */
     ...
     * 输出: "ball" 解释: "hit" 出现了3次，但它是一个禁用的单词。 "ball" 出现了2次 (同时没有其他单词出现2次)，所以它是段落里出现次数最多的，且不在禁用列表中的单词。
     * 注意，所有这些单词在段落里不区分大小写，标点符号需要忽略（即使是紧挨着单词也忽略， 比如 "ball,"）， "hit"不是最终的答案，虽然它出现次数更多，但它在禁用单词列表中。
     * <p>
     * 提交次数:4 解决方式: 自我解答 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param paragraph
     * @param banned
     * @return
     ...
     * <p>
     * <p>
     * 输入：nums = [1,2,2,4] 输入：nums = [1,2,2,4,5,6,7] 输出：[2,3]
     * <p>
     * 提交次数:6 解决方式: 自我解答 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     * @return
     */
     ...
     * <p>
     * 提交次数:2 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     *
     * @param name
     * @param typed
     * @return
    ...
    /**
     * 句子仅由小写字母（'a' 到 'z'）、数字（'0' 到 '9'）、连字符（'-'）、标点符号（'!'、'.' 和 ','）以及空格（' '）组成。每个句子可以根据空格分解成
     * 一个或者多个 token ，这些 token 之间由一个或者多个空格 ' ' 分隔。
     * <p>
     * 如果一个 token 同时满足下述条件，则认为这个 token 是一个有效单词：
     * <p>
     * 仅由小写字母、连字符和/或标点（不含数字）组成。 至多一个 连字符 '-' 。如果存在，连字符两侧应当都存在小写字母（"a-b" 是一个有效单词，但 "-ab" 和 "ab-"
     * 不是有效单词）。 至多一个 标点符号。如果存在，标点符号应当位于 token 的 末尾 。 这里给出几个有效单词的例子："a-b."、"afad"、"ba-c"、"a!" 和 "!"
     * 。
     * <p>
     * 给你一个字符串 sentence ，请你找出并返回 sentence 中 有效单词的数目 。
     * <p>
     * 输入：sentence = "alice and  bob are playing stone-game10" 输出：5 解释：句子中的有效单词是
     * "alice"、"and"、"bob"、"are" 和 "playing" "stone-game10" 不是有效单词，因为它含有数字
     * <p>
     * 提交次数:7 解决方式: 自我解决 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param sentence
     * @return
     */
     ...
     * <p>
     * 输入：s = "luffy is still joyb1oy  " 输出：6 解释：最后一个单词是长度为6的“joyboy”。
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param s
     ...
     * <p>
     * 提交次数:2 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param arr
     * @return
     */
     ...
     * 。
     * <p>
     * 请你重新排列空格，使每对相邻单词之间的空格数目都 相等 ，并尽可能 最大化 该数目。如果不能重新平均分配所有空格，请 将多余的空格放置在字符串末尾 ，这也意味着返回的字符串应当与原
     * text 字符串的长度相等。
     * <p>
     * 返回 重新排列空格后的字符串 。
     * <p>
     * 输入：text = " practice   makes   perfect" 输出："practice   makes   perfect " 解释：总共有 7 个空格和 3
     * 个单词。7 / (3-1) = 3 个空格加上 1 个多余的空格。多余的空格需要放在字符串的末尾。
     * <p>
     * 提交次数:3 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param text
     ...
     * <p>
     * 形式上，如果可以找出索引 i + 1 < j 且满足 (arr[0] + arr[1] + ... + arr[i] == arr[i + 1] + arr[i + 2] + ... +
     ...
     * <p>
     * 输入：arr = [1,2,1,2,1,2,1,3], m = 2, k = 2 输出：true 解释：模式 (1,2) 长度为 2 ，且连续重复 2 次。另一个符合题意的模式是
     * (2,1) ，同样重复 2 次。
     * <p>
     * 提交次数:3 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     ...
     * <p>
     * 输入：nums = [1,2,10,5,7] 输出：true 解释：从 nums 中删除下标 2 处的 10 ，得到 [1,2,5,7] 。 [1,2,5,7] 是严格递增的，所以返回
     * true 。
     * <p>
     * <p>
     * [105,924,32,968] [2,3,1,2]
     * <p>
     * <p>
     * 提交次数:3 解决方式: 参考答案 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param nums
     * @return
     */
     ...
     * <p>
     * 给你一个整数数组  flowerbed 表示花坛，由若干 0 和 1 组成，其中 0 表示没种植花，1 表示种植了花。另有一个数 n ，能否在不打破种植规则的情况下种入 n
     * 朵花？能则返回 true ，不能则返回 false。
     * <p>
     * 输入：flowerbed = [1,0,0,0,0,0,0,0,1], n = 3 输出：true
     * <p>
     * 提交次数:6 解决方式: 自我解决 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param flowerbed
     * @param n
     * @return
     ...
     * <p>
     * 提交次数:5 解决方式: 自我解决 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param s
     * @param goal
     * @return
    ...
    public boolean buddyStrings(String s, String goal) {
        ...
        for (int i = 0; i < goal.length(); i++) {
            if (i - 1 >= 0 && chars[i] != chars[i - 1] && flag) {
                flag = false;
            }
     ...
     * <p>
     * 请你用空格替换每个不是数字的字符。例如，"a123bc34d8ef34" 将会变成 " 123  34 8  34" 。注意，剩下的这些整数为（相邻彼此至少有一个空格隔开）："123"、"34"、"8"
     * 和 "34" 。
     * <p>
     ...
     * <p>
     * 如果对于所有 i <= j，nums[i] <= nums[j]，那么数组 nums 是单调递增的。 如果对于所有 i <= j，nums[i]> = nums[j]，那么数组 nums
     * 是单调递减的。
     * <p>
     ...
     * <p>
     * 输入：nums = [11,7,2,15] 输出：2 解释：元素 7 ：严格较小元素是元素 2 ，严格较大元素是元素 11 。 元素 11 ：严格较小元素是元素 7 ，严格较大元素是元素
     * 15 。 总计有 2 个元素都满足在 nums 中同时存在一个严格较小元素和一个严格较大元素。
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     ...
     * <p>
     * 给你一个长度为 n 的字符串 keysPressed ，其中 keysPressed[i] 表示测试序列中第 i 个被按下的键。releaseTimes 是一个升序排列的列表，其中
     * releaseTimes[i] 表示松开第 i 个键的时间。字符串和数组的 下标都从 0 开始 。第 0 个键在时间为 0 时被按下，接下来每个键都 恰好 在前一个键松开时被按下。
     * <p>
     * 测试人员想要找出按键 持续时间最长 的键。第 i 次按键的持续时间为 releaseTimes[i] - releaseTimes[i - 1] ，第 0 次按键的持续时间为
     * releaseTimes[0] 。
     * <p>
     ...
     * <p>
     * 输入：releaseTimes = [9,29,49,50], keysPressed = "cbcd" 输出："c" 解释：按键顺序和持续时间如下： 按下 'c' ，持续时间 9（时间
     * 0 按下，时间 9 松开） 按下 'b' ，持续时间 29 - 9 = 20（松开上一个键的时间 9 按下，时间 29 松开） 按下 'c' ，持续时间 49 - 29 =
     * 20（松开上一个键的时间 29 按下，时间 49 松开） 按下 'd' ，持续时间 50 - 49 = 1（松开上一个键的时间 49 按下，时间 50 松开） 按键持续时间最长的键是
     * 'b' 和 'c'（第二次按下时），持续时间都是 20 'c' 按字母顺序排列比 'b' 大，所以答案是 'c'
     * <p>
     * 提交次数:4 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param releaseTimes
     * @param keysPressed
     * @return
     ...
     * <p>
     * 全部字母都是大写，比如 "USA" 。 单词中所有字母都不是大写，比如 "leetcode" 。 如果单词不只含有一个字母，只有首字母大写， 比如 "Google" 。 给你一个字符串
     * word 。如果大写用法正确，返回 true ；否则，返回 false 。
     * <p>
     * 输入：word = "USA" 输出：true
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
    ...
    public boolean detectCapitalUse(String word) {
        if (checkCapital(word) || (word.charAt(0) >= 'A' && word.charAt(0) <= 'Z' && checkCapital(
            word.substring(1)))) {
            return true;
        }
        return false;
     ...
     * <p>
     * 输入:list1 = ["Shogun", "Tapioca Express", "Burger King", "KFC"]，list2 = ["KFC", "Shogun",
     * "Burger King"] 输出: ["Shogun"] 解释: 他们共同喜爱且具有最小索引和的餐厅是“Shogun”，它有最小的索引和1(0+1)。
     * <p>
     * 提交次数:5 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param list1
     ...
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param arr
     * @param k
     * @return
     ...
     * <p>
     * nums[0] = 0 nums[1] = 1 当 2 <= 2 * i <= n 时，nums[2 * i] = nums[i] 当 2 <= 2 * i + 1 <= n
     * 时，nums[2 * i + 1] = nums[i] + nums[i + 1] 返回生成数组 nums 中的 最大 值。
     * <p>
     * 输入：n = 7 输出：3 解释：根据规则： nums[0] = 0 nums[1] = 1 nums[(1 * 2) = 2] = nums[1] = 1 nums[(1 * 2) +
     * 1 = 3] = nums[1] + nums[2] = 1 + 1 = 2 nums[(2 * 2) = 4] = nums[2] = 1 nums[(2 * 2) + 1 = 5]
     * = nums[2] + nums[3] = 1 + 2 = 3 nums[(3 * 2) = 6] = nums[3] = 2 nums[(3 * 2) + 1 = 7] =
     * nums[3] + nums[4] = 2 + 1 = 3 因此，nums = [0,1,1,2,1,3,2,3]，最大值 3
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param n
     ...
     * <p>
     * 输入：s = "iloveleetcode", words = ["i","love","leetcode","apples"] 输出：true 解释： s 可以由 "i"、"love"
     * 和 "leetcode" 相连得到。
     * <p>
     * 提交次数:6 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param s
     * @param words
     * @return
    ...
    /**
     * 给你一个下标从 0 开始的整数数组 nums 。如果两侧距 i 最近的不相等邻居的值均小于 nums[i] ，则下标 i 是 nums 中，某个峰的一部分。类似地，如果两侧距 i
     * 最近的不相等邻居的值均大于 nums[i] ，则下标 i 是 nums 中某个谷的一部分。对于相邻下标 i 和 j ，如果 nums[i] == nums[j] ， 则认为这两下标属于
     * 同一个 峰或谷。
     * <p>
     ...
     * 返回 nums 中峰和谷的数量。
     * <p>
     * 输入：nums = [2,4,1,1,6,5] 输出：3 解释： 在下标 0 ：由于 2 的左侧不存在不相等邻居，所以下标 0 既不是峰也不是谷。 在下标 1 ：4 的最近不相等邻居是
     * 2 和 1 。由于 4 > 2 且 4 > 1 ，下标 1 是一个峰。 在下标 2 ：1 的最近不相等邻居是 4 和 6 。由于 1 < 4 且 1 < 6 ，下标 2 是一个谷。
     * 在下标 3 ：1 的最近不相等邻居是 4 和 6 。由于 1 < 4 且 1 < 6 ，下标 3 符合谷的定义，但需要注意它和下标 2 是同一个谷的一部分。 在下标 4 ：6
     * 的最近不相等邻居是 1 和 5 。由于 6 > 1 且 6 > 5 ，下标 4 是一个峰。 在下标 5 ：由于 5 的右侧不存在不相等邻居，所以下标 5 既不是峰也不是谷。 共有 3
     * 个峰和谷，所以返回 3 。
     * <p>
     * 提交次数:5 解决方式: 自我解决+参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     *
     * @param nums
     * @return
     */
        ...
        for (int i = 1; i < nums.length - 1; i++) {
            ...
            while (left >= 0 && right < nums.length) {
                if (nums[i] == nums[left]) {
                    ...
                    if ((nums[i] > nums[left] && nums[i] > nums[right]) || (nums[i] < nums[left]
                        && nums[i] < nums[right])) {
                        if (nums[i - 1] != nums[i]) {
                            count++;
                        }
     ...
     * <p>
     * 输入：bills = [5,5,5,10,20] 输出：true 解释： 前 3 位顾客那里，我们按顺序收取 3 张 5 美元的钞票。 第 4 位顾客那里，我们收取一张 10
     * 美元的钞票，并返还 5 美元。 第 5 位顾客那里，我们找还一张 10 美元的钞票和一张 5 美元的钞票。 由于所有客户都得到了正确的找零，所以我们输出 true。
     *
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     ...
     * <p>
     * 使用当前带宽下载插件 将带宽加倍（下载插件数量随之加倍） 请返回小扣完成下载 n 个插件最少需要多少分钟。
     * <p>
     ...
     * <p>
     * <p>
     * 输入：nums = [0,1,2,4,6,7] 输出：["0->2","4->5","7"] 解释：区间范围是： [0,2] --> "0->2" [4,5] --> "4->5"
     * [7,7] --> "7"
     * <p>
     * 提交次数:4 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
    ...
    private String getTargetString(int start, int end) {
        StringBuffer buffer = new StringBuffer();
        if (start == end) {
            return buffer.append(start).toString();
        }
        buffer.append(start);
        buffer.append("->");
        buffer.append(end);
     ...
     * <p>
     * 对每个孩子 i，都有一个胃口值 g[i]，这是能让孩子们满足胃口的饼干的最小尺寸；并且每块饼干 j，都有一个尺寸 s[j] 。如果 s[j] >= g[i]，我们可以将这个饼干 j
     * 分配给孩子 i ，这个孩子会得到满足。你的目标是尽可能满足越多数量的孩子，并输出这个最大数值。
     * <p>
     * 输入: g = [1,6], s = [1,2,3] 输出: 2 解释: 你有两个孩子和三块小饼干，2个孩子的胃口值分别是1,2。 你拥有的饼干数量和尺寸都足以让所有孩子满足。
     * 所以你应该输出2.
     * <p>
     * 提交次数:1 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     *
     ...
     * <p>
     * 输入：nums = [0,1,0,0,0,1,0,0,1], k = 2 输出：true 解释：每个 1 都至少相隔 2 个元素。
     * <p>
     * 提交次数:1 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     * @param k
     * @return
     ...
     * <p>
     * 输入：s = "XXXOOXXX" 输出：2 解释：XXOX -> OOOX -> OOOO 第一次操作，选择前 3 个字符，并将这些字符转换为 'O' 。 然后，选中后 3
     * 个字符，并执行转换。最终得到的字符串全由字符 'O' 组成。
     * <p>
     * 提交次数:3 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param s
    ...
    public int minimumMoves_unfinished(String s) {
        ...
        for (int i = 0; i < s.length(); i += 3) {
            if (s.substring(i, i + 3 < s.length() ? i + 3 : s.length()).contains("X")) {
                count++;
            }
        }
        return count;
    ...
    /**
     * 环形公交路线上有 n 个站，按次序从 0 到 n - 1 进行编号。我们已知每一对相邻公交站之间的距离，distance[i] 表示编号为 i 的车站和编号为 (i + 1) % n
     * 的车站之间的距离。
     * <p>
     ...
     * <p>
     * 输入：distance = [1,2,3,4], start = 1, destination = 2 输出：2 解释：公交站 1 和 2 之间的距离是 2 或 8，最小值是 2。
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param distance
     ...
     * <p>
     * 0 <= i <= n - 2 nums[i] == key 且 nums[i + 1] == target 。 请你返回出现 最多 次数的 target 。测试数据保证出现次数最多的
     * target 是唯一的。
     * <p>
     * 输入：nums = [1,100,200,1,300,300,300], key = 1 输出：100 解释：对于 target = 100 ，在下标 1 和 4 处出现过 2
     * 次，且都紧跟着 key 。 没有其他整数在 key 后面紧跟着出现，所以我们返回 100 。
     * <p>
     * 提交次数:3 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     ...
     * <p>
     * 输入：nums = [1,2,2,3,1] 输出：2 解释： 输入数组的度是 2 ，因为元素 1 和 2 的出现频数最大，均为 2 。 连续子数组里面拥有相同度的有如下所示： [1,
     * 2, 2, 3, 1], [1, 2, 2, 3], [2, 2, 3, 1], [1, 2, 2], [2, 2, 3], [2, 2] 最短连续子数组 [2, 2] 的长度为 2
     * ，所以返回 2 。
     *
     * <p>
     * 提交次数:3 解决方式: 自我解决 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     ...
     * <p>
     * 输入：words = ["word","world","row"], order = "worldabcefghijkmnpqstuvxyz" 输出：false
        ...
        for (int i = 0; i < words.length - 1; i++) {
            ...
            while (index < end) {
                if (word.charAt(index) == next.charAt(index)) {
                    index++;
                } else if (order.indexOf(word.charAt(index) + "") > order
                    .indexOf(next.charAt(index) + "")) {
                    return false;
                } else {
                    break;
                }
                if (index == end && word.length() > next.length()) {
                    return false;
                }
            }
        }
    ...
    /**
     * 给你一个下标从 0 开始的整数数组 nums ，该数组的大小为 n ，请你计算 nums[j] - nums[i] 能求得的 最大差值 ，其中 0 <= i < j < n 且
     * nums[i] < nums[j] 。
     * <p>
     * 返回 最大差值 。如果不存在满足要求的 i 和 j ，返回 -1 。
     * <p>
     * 输入：nums = [7,1,5,4] 输出：4 解释： 最大差值出现在 i = 1 且 j = 2 时，nums[j] - nums[i] = 5 - 1 = 4 。 注意，尽管 i
     * = 1 且 j = 0 时 ，nums[j] - nums[i] = 7 - 1 = 6 > 4 ，但 i > j 不满足题面要求，所以 6 不是有效的答案。
     * <p>
     * 提交次数:2 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param nums
     ...
     * <p>
     * <p>
     * 提交次数:4 解决方式: 参考答案 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param s
     * @param k
     * @return
     ...
     * 返回艾希处于中毒状态的 总 秒数。
     * <p>
     * 输入：timeSeries = [1,2], duration = 2 输出：3 解释：提莫攻击对艾希的影响如下： - 第 1 秒，提莫攻击艾希并使其立即中毒。中毒状态会维持 2
     * 秒，即第 1 秒和第 2 秒。 - 第 2 秒，提莫再次攻击艾希，并重置中毒计时器，艾希中毒状态需要持续 2 秒，即第 2 秒和第 3 秒。 艾希在第 1、2、3
     * 秒处于中毒状态，所以总中毒秒数是 3 。
     * <p>
     * 提交次数:2 解决方式: 自我解决 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     * <p>
     ...
     * <p>
     * 输入：ranges = [[1,2],[3,4],[5,6]], left = 2, right = 5 输出：true 解释：2 到 5 的每个整数都被覆盖了： - 2
     * 被第一个区间覆盖。 - 3 和 4 被第二个区间覆盖。 - 5 被第三个区间覆盖。
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param ranges
     ...
     * 输入：n = 25 输出：1389537
     * <p>
     * 提交次数:2 解决方式: 参考答案 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param n
     * @return
     */
    ...
    public int tribonacci_unfinished(int n) {
        ...
        return tribonacci_unfinished(n - 1) + tribonacci_unfinished(n - 2) + tribonacci_unfinished(
            n - 3);
    }
     ...
     * <p>
     * 输入：s = "ilovecodingonleetcode", target = "code" 输出：2 解释： 对于 "code" 的第 1 个副本，选取下标为 4 、5 、6 和 7
     * 的字符。 对于 "code" 的第 2 个副本，选取下标为 17 、18 、19 和 20 的字符。 形成的字符串分别是 "ecod" 和 "code" ，都可以重排为 "code" 。
     ...
     * <p>
     * 输入：title = "First leTTeR of EACH Word" 输出："First Letter of Each Word" 解释： 单词 "of" 长度为 2
     * ，所以它保持完全小写。 其他单词长度都至少为 3 ，所以其他单词首字母大写，剩余字母小写。
     * <p>
     * 提交次数:2 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param title
     ...
     * <p>
     * 输入：words = ["mass","as","hero","superhero"] 输出：["as","hero"] 解释："as" 是 "mass" 的子字符串，"hero" 是
     * "superhero" 的子字符串。 ["hero","as"] 也是有效的答案。
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param words
    ...
    private void setValue(List<List<Integer>> ret, int start, int end) {
        if (end - start >= 3) {
     ...
     * 子字符串 是字符串中的一个连续字符序列。
     * <p>
     * 输入：s = "abdcabbacgq" 输出：4 解释：最优的子字符串是 "abba" ，其他的非最优解包括 "bb" 和 "" 。
     ...
     * <p>
     * 输入：word = "cuaieuouac" 输出：7 解释：下面列出 word 中的元音子字符串（斜体加粗部分）： - "cuaieuouac" - "cuaieuouac" -
     * "cuaieuouac" - "cuaieuouac" - "cuaieuouac" - "cuaieuouac" - "cuaieuouac"
     * <p>
     * 提交次数:1 解决方式: 参考答案 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     * <p>
     * uaieuoua aieuoua ieuoua uaieuou uaieuo aieuou aieuo
     *
     * @param word
     * @return
     */
        ...
        for (int r = 0; r < word.length(); ++r) {            //枚举右指针
            ...
            if (vowels.contains(c)) {
                ...
                if (isLegal()) {                            //若子串合法，计算以当前右指针所在字符结束的合法子串数
                    if (prev == 0) {
                        prev = 1;                //若以上一个字符结束无合法子串，则初始子串数为1，否则为上一个字符结束的合法子串数
                    }
    ...
    /**
     * 给你一个字符串 num ，表示一个大整数。如果一个整数满足下述所有条件，则认为该整数是一个 优质整数 ： 该整数是 num 的一个长度为 3 的 子字符串 。 该整数由唯一一个数字重复
     * 3 次组成。 以字符串形式返回 最大的优质整数 。如果不存在满足要求的整数，则返回一个空字符串 "" 。 注意： 子字符串 是字符串中的一个连续字符序列。 num 或优质整数中可能存在
     * 前导零 。 输入：num = "6777133339" 输出："777" 解释：num 中存在两个优质整数："777" 和 "333" 。 "777" 是最大的那个，所以返回 "777"
     * 。
     * <p>
     * 提交次数:1 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
    ...
    public String dayOfTheWeek_unfinished(int day, int month, int year) throws Exception {
        ...
        Date parse = simpleDateFormat.parse(
            new StringBuffer().append(year).append("-").append(month).append("-").append(day)
                .toString());
    ...
    /**
     * 给你一个字符串 sentence 作为句子并指定检索词为 searchWord ，其中句子由若干用 单个空格 分隔的单词组成。请你检查检索词 searchWord 是否为句子
     * sentence 中任意单词的前缀。
     * <p>
     ...
     * <p>
     * 输入：sentence = "this problem is an easy problem", searchWord = "pro" 输出：2 解释："pro" 是 "problem"
     * 的前缀，而 "problem" 是句子中第 2 个也是第 6 个单词，但是应该返回最小下标 2 。
     * <p>
     * 提交次数:5 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param sentence
     ...
     * <p>
     * 输入：nums = [1,2,3] 输出：3 解释： 只需要3次操作（注意每次操作会增加两个元素的值）： [1,2,3]  =>  [2,3,3]  =>  [3,4,3]  =>
     * [4,4,4]
     * <p>
     * 提交次数:1 解决方式: 参考答案 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     ...
     * [9,4,1,7] 最高分和最低分之间的差值是 9 - 1 = 8 - [9,4,1,7] 最高分和最低分之间的差值是 9 - 7 = 2 - [9,4,1,7]
     * 最高分和最低分之间的差值是 4 - 1 = 3 - [9,4,1,7] 最高分和最低分之间的差值是 7 - 4 = 3 - [9,4,1,7] 最高分和最低分之间的差值是 7 - 1 =
     * 6 可能的最小差值是 2
     * <p>
     * 提交次数:2 解决方式: 参考答案 未来是否需要更优化的解题方法:否 未来是否需要复盘:是
     * <p>
    ...
    /**
     * 有 n 个人前来排队买票，其中第 0 人站在队伍 最前方 ，第 (n - 1) 人站在队伍 最后方 。 给你一个下标从 0 开始的整数数组 tickets ，数组长度为 n ，其中第 i
     * 人想要购买的票数为 tickets[i] 。 每个人买票都需要用掉 恰好 1 秒 。一个人 一次只能买一张票 ，如果需要购买更多票，他必须走到  队尾 重新排队（瞬间
     * 发生，不计时间）。如果一个人没有剩下需要买的票，那他将会 离开 队伍。 返回位于位置 k（下标从 0 开始）的人完成买票需要的时间（以秒为单位）。
     * <p>
     * 输入：tickets = [5,1,1,1], k = 0 输出：8 解释： - 第一轮，队伍中的每个人都买到一张票，队伍变为 [4, 0, 0, 0] 。 - 接下来的 4
    ...
    /**
     * 给你一个整数数组 cost ，其中 cost[i] 是从楼梯第 i 个台阶向上爬需要支付的费用。一旦你支付此费用，即可选择向上爬一个或者两个台阶。 你可以选择从下标为 0 或下标为 1
     * 的台阶开始爬楼梯。 请你计算并返回达到楼梯顶部的最低花费。
     * <p>
     * 输入：cost = [1,100,1,1,1,100,1,1,100,1] 输出：6 解释：你将从下标为 0 的台阶开始。 - 支付 1 ，向上爬两个台阶，到达下标为 2 的台阶。 -
     * 支付 1 ，向上爬两个台阶，到达下标为 4 的台阶。 - 支付 1 ，向上爬两个台阶，到达下标为 6 的台阶。 - 支付 1 ，向上爬一个台阶，到达下标为 7 的台阶。 - 支付 1
     * ，向上爬两个台阶，到达下标为 9 的台阶。 - 支付 1 ，向上爬一个台阶，到达楼梯顶部。 总花费为 6 。
     * <p>
     * 提交次数:1 解决方式: 参考答案 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
     *
     * @param cost
    ...
    public int minCostClimbingStairs_unfinished(int[] cost) {
        ...
        int start = Math.min(cost[0] + cost[1], cost[0] + cost[2])
            < Math.min(cost[1] + cost[2], cost[1]) + (cost.length > 3 ? cost[3] : 0) ? cost[0]
            : cost[1];
     ...
     * 请你按下述方式重新格式化电话号码。
     * <p>
     * 首先，删除 所有的空格和破折号。 其次，将数组从左到右 每 3 个一组 分块，直到 剩下 4 个或更少数字。剩下的数字将按下述规定再分块： 2 个数字：单个含 2 个数字的块。 3
     * 个数字：单个含 3 个数字的块。 4 个数字：两个分别含 2 个数字的块。 最后用破折号将这些块连接起来。注意，重新格式化过程中 不应该 生成仅含 1 个数字的块，并且 最多 生成两个含
     * 2 个数字的块。
     * <p>
     * 返回格式化后的电话号码。
     * <p>
     * 输入：number = "123 4-567" 输出："123-45-67" 解释：数字是 "1234567". 步骤 1：共有超过 4 个数字，所以先取 3 个数字分为一组。第 1
     * 个块是 "123" 。 步骤 2：剩下 4 个数字，所以将它们分成两个含 2 个数字的块。这 2 块分别是 "45" 和 "67" 。 连接这些块后得到 "123-45-67" 。
     * <p>
     * 提交次数:2 解决方式: 自我解决 未来是否需要更优化的解题方法:否 未来是否需要复盘:否
     *
     * @param number
    ...
    public String reformatNumber(String number) {
        ...
        if (replace.length() % 3 == 0) {
            len = replace.length() / 3 - 1;
        } else {
            if (replace.length() % 3 == 4) {
                len = replace.length() / 3 + 1;
            } else {
                len = replace.length() / 3;
            }
        }
        char[] retChars = new char[len + replace.length()];
        int cnt = 0;
        for (int i = 0, j = 0; i < retChars.length && j < replace.length(); i++) {
            if (replace.length() % 3 == 1 && (len + replace.length() - i) <= 5) {
                if (cnt == 2) {
                    retChars[i] = '-';
                    cnt = 0;
                } else {
                    retChars[i] = replace.charAt(j++);
                    cnt++;
                }
            } else {
                if ((i + 1) % 4 == 0) {
     ...
     * <p>
     * 它有至少 8 个字符。 至少包含 一个小写英文 字母。 至少包含 一个大写英文 字母。 至少包含 一个数字 。 至少包含 一个特殊字符 。特殊字符为："!@#$%^&*()-+"
     * 中的一个。 它 不 包含 2 个连续相同的字符（比方说 "aab" 不符合该条件，但是 "aba" 符合该条件）。 给你一个字符串 password ，如果它是一个 强 密码，返回
     * true，否则返回 false 。
     * <p>
     * <p>
    ...
    public boolean strongPasswordCheckerII(String password) {
        ...
        if (count['!'] > 0 || count['@'] > 0 || count['#'] > 0 || count['$'] > 0 || count['%'] > 0
        ...
        for (int i = 0; i < count.length; i++) {
            if (count[i] > 1) {
                if (isRepeat((char) i, password, count[i])) {
                    return false;
                }
            }
        }
    ...
    public boolean strongPasswordCheckerII_unfinished(String password) {
        ...
        return password.matches("[a-z]+") && password.matches("[A-Z]+") && password
            .matches("[0-9]+") && password.matches("[\\!\\@\\#\\$\\%\\^\\&\\*\\(\\)\\-\\+]+")
            && !password.matches("(.)\\1{1}") && password.length() >= 8;
    }
     ...
     * <p>
     * 例如，数组 [2,5,6] 的 异或总和 为 2 XOR 5 XOR 6 = 1 。 给你一个数组 nums ，请你求出 nums 中每个 子集 的 异或总和 ，计算并返回这些值相加之
     * 和 。
     * <p>
     ...
     * <p>
     * 输入：nums = [5,1,6] 输出：28 解释：[5,1,6] 共有 8 个子集： - 空子集的异或总和是 0 。 - [5] 的异或总和为 5 。 - [1] 的异或总和为 1
     * 。 - [6] 的异或总和为 6 。 - [5,1] 的异或总和为 5 XOR 1 = 4 。 - [5,6] 的异或总和为 5 XOR 6 = 3 。 - [1,6] 的异或总和为 1
     * XOR 6 = 7 。 - [5,1,6] 的异或总和为 5 XOR 1 XOR 6 = 2 。 0 + 5 + 1 + 6 + 4 + 3 + 7 + 2 = 28
     * <p>
     * TODO
     * <p>
     * 提交次数:1 解决方式: 参考答案 未来是否需要更优化的解题方法:是 未来是否需要复盘:是
    ...
    public int minimumSum(int num) {
        ...
        Arrays.sort(chars);
        return Integer.parseInt(chars[0] + "" + chars[2]) + Integer
            .parseInt(chars[1] + "" + chars[3]);
    }
